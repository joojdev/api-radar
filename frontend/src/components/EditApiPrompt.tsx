...
export default function EditApiPrompt() {
  ...
  const [protocol, setProtocol] = useState<Protocol>(
    apiList[dropDownSelected].protocol,
  );
  const [domain, setDomain] = useState<string>(
    apiList[dropDownSelected].domain,
  );
  const [endpoint, setEndpoint] = useState<string>(
    apiList[dropDownSelected].endpoint,
  );
  const [accessInterval, setAccessInterval] = useState<number>(
    apiList[dropDownSelected]?.accessInterval,
  );
  const [errors, setErrors] = useState<
    Partial<Record<keyof EditApiInput, string>>
  >({});
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    const data: EditApiInput = {
      id: apiList[dropDownSelected].id!,
      name,
    ...
    try {
      ...
      const message =
        ...
        "There was an error while editing the API!";
      toast.error(message);
    }
